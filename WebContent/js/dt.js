		...
		$(function(){
			...
			$.getJSON("ml/dTree/getTrainingDataSets",{id: $(this).val(), ajax: 'true'}, function(j){
		 		var $subType = $("#e1");
				$subType.empty();
				$.each(j, function () {
			...
			$.getJSON("ml/dTree/getTestDataSets",{id: $(this).val(), ajax: 'true'}, function(j){
		 		var $subType = $("#e2");
				$subType.empty();
				$.each(j, function () {
					$subType.append($('<option></option>').attr("value", this.optionValue).text(this.optionDisplay));
		...
		$("#btn-chart").click(function() {
			...
			setTimeout(function() { 
				$.ajax({
					type: "GET",
					dataType: "text",
					url: "ml/dTree/getReport/groups.train-groups.test.chart",
					success: function (d) {
						a.button("complete");
		...
		$("#dtForm button").click(function(event) {
			...
			a.button("loading");
			var myUrl = "ml/dTree/runDistributedService"; 
