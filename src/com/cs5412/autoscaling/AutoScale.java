...
/**
 * <p>
 * <b>This class provides the autoscaling functionality.Choose a random server
 * from a list of servers marked as 'MAINT' and change their status to 'UP', so
 * that they can start receiving connections.</b>
 * </p>
 * 
 * @author kt466
 * 
...
public class AutoScale {
	...
	public AutoScale(PropertiesConfiguration cfg) {
		this.NODE_NAME = cfg.getString("NODE_NAME");
	...
	/**
	 * Choose a random server from a list of servers marked as 'MAINT' and
	 * change their status to 'UP', so that they can start receiving
	 * connections.
	 * 
	 * @throws Exception
	 */
	public void scaleUp() throws Exception {
		...
		availableServers.remove(this.NODE_NAME);
		String availableServer = availableServers.get(randomGenerator
				.nextInt(availableServers.size()));
		String cmd = "echo \"enable server " + SERVER_POOL_NAME + "/"
				+ availableServer + "\" | socat stdio " + HASOCKET;
		lbShell.execute(cmd);
		lbShell.disconnect();
	...
	public static void main(String args[]) throws Exception {
		CMD_DISABLE = "echo \"disable server " + "servers/node2"
				+ "\" | socat stdio " + HASOCKET;
		CMD_ENABLE = "echo \"enable server " + "servers/node1"
				+ "\" | socat stdio " + HASOCKET;
